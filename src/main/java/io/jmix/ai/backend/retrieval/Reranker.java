...
public class Reranker {
    ...
    public Reranker(
            @Value("${reranker.base-url:http://localhost:8000}") String rerankerBaseUrl
    ) {
        this.restTemplate = new RestTemplate();
        // Ensure URL ends with /rerank
        this.rerankerServiceUrl = rerankerBaseUrl.endsWith("/rerank")
                ? rerankerBaseUrl
    ...
    public List<Result> rerank(String query, List<Document> documents, int topN) {
        ...
        log.info("Sending rerank request: query={}, documents={}",
                query, documents.stream().map(doc -> getUrlOrSource(doc)).toList());
