...
- `JavaApiIngester`: loads Java API reference from the Jmix Javadoc site and renders each class page as a compact "API card" snippet with verbatim signatures. With `javaapi.enrichment.enabled` an LLM additionally generates a description and a usage example for each card; generated content is cached in the database by source hash, so unchanged pages are not re-generated. Only versions with a configured Javadoc base URL are ingested. Configured by the `javaapi.*` application properties.
- `DocsSnippetsIngester` and `UiSamplesSnippetsIngester`: separate corpora (`docs-snippets`, `uisamples-snippets`) that coexist in the vector store with the raw `docs` and `uisamples` corpora and are updated independently. The same source pages are converted into small context7-like snippets by an LLM, plus lossless plain-text coverage chunks of each page — retrieval returns both. Generation is cached like the Java API enrichment and configured by the `snippets.enrichment.*` properties. The default chat/search parameters already point the retrieval tools at these corpora; the `tools.<name>.vectorType` parameter switches a tool back to a raw corpus (`docs`, `uisamples`).
